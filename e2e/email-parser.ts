export function parseCandidateEmailHtml(emailContent:string) {
    ...
    const candidateParts = [...emailContent.replace(/[\r\n]/g, '').matchAll(/Potential teacher in ([^<]+)<(.*?)(?=Potential teacher in|$)/gm)]

    for(const part of candidateParts) {
        const items = [...part[2].matchAll(/<dt[^>]*>([^<]*)<\/dt>\s*<dd[^>]*>(.*?)<\/dd>/gm)];
        ...
        const result = {
            ...
            colleges: itemDict['Colleges they have matched with'].split(',').map(s => s.trim()),
