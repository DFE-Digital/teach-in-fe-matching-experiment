...
app.http('register-candidate', {
    ...
    handler: async (request, context) => {
        ...
        const candidateContactId = await getContactId(requestData.email, context);
        ...
        let reachableColleges = await getClosestColleges(candidateLat, candidateLong, context);
        
        // sort colleges by distance
        let ordredReachableColleges = reachableColleges.sort((a, b) => a.reachableColleges - b.reachableColleges);
        if(ordredReachableColleges != undefined && ordredReachableColleges.length > 0) {
            let collegeIds = []; 
            let chosenGroupIds = [];
            let waitingListGroup = [];
            let collegeCount = 0;
            for (let i = 0; i < ordredReachableColleges.length; i++) {
                if (collegeCount < 5) { // do the following until we get to 5 colleges 
                    if(!hasGroupChosenBefore(chosenGroupIds, ordredReachableColleges[i].collegeGroupId)) {
                        ...
                        chosenGroupIds.push(ordredReachableColleges[i].collegeGroupId);
                        collegeCount++;
                    } else {
                        waitingListGroup.push({
                            "collegeGroupId": ordredReachableColleges[i].collegeGroupId,
            ...
            setCollegeGroupToNeedsInvite(collegeGroups, chosenGroupIds, context);
            
            assignToCollege(collegeIds, candidateContactId, context, candidateLat, candidateLong);
        }
...
async function getCollegeGroups(context) {
    let collegeGroup = []
    await axios.get(urls.group(), { headers: urls.qualtricsHeader()})
        .then( (response) => {
            collegeGroup = response.data.result.elements;
        })
        .catch((error) => {
            context.log.error(`Error while trying to fetch college-groups: `, error);
        });
...
async function getClosestColleges(candidateLat, candidateLong, context) {
    let closestColleges = []
    await axios.get(urls.college(), { headers: urls.qualtricsHeader()})
        .then( (response) => {
...
function assignToCollege(collegeIds, candidateContactId, context, candidateLat, candidateLong) {
...
function setCollegeGroupToNeedsInvite(collegeGroups, chosenGroupIds, context) {
    ...
    for (let i = 0; i < collegeGroups.length; i++) {
        if (chosenGroupIds.includes(collegeGroups[i].extRef) && collegeGroups[i].embeddedData.groupStatus == 'Unregistered') {
            ...
            const update_college_URL = urls.updateCollegeGroup()+'/'+collegeGroups[i].contactId;
            axios.put(update_college_URL, current_college, { headers: urls.qualtricsHeader()});
        }
    }
}
...
async function getContactId(candidateEmail, context) {
    ...
    let contactId = '';
    await axios.get(urls.candidate(), { headers: urls.qualtricsHeader()})
        .then( (response) => {
