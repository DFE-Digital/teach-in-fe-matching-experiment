...
app.http("prepare-report", {
    ...
    handler: async (request, context) => {
        ...
        for (let y = 0; y < activeCollegeGroups.length; y++) {
            ...
            if (activeCollegeGroups[y].unsubscribed)
                unsubscribed++;
        }
        ...
        console.log(''); 
        console.log('------------------------------------------------------------------------------------------------------------------');
        console.log('                                         College status report'); 
        console.log('------------------------------------------------------------------------------------------------------------------');
        console.log('                                         Needs Invite = ' + needsInvite);
        console.log('                                         Invited = ' + invited);
        console.log('                                         Signed up = ' + active);
...
function displayResultByRegion(region: Candidate[], regionName: string) {
        ...
        for (let i = 0; i < region.length; i++) {
            ...
            if(region[i].embeddedData.collegeGroup5Id != undefined)
                fiveMatches++;
            if(region[i].embeddedData.collegeGroup4Id != undefined && region[i].embeddedData.collegeGroup5Id == undefined)
                fourMatches++;
            if(region[i].embeddedData.collegeGroup3Id != undefined && region[i].embeddedData.collegeGroup4Id == undefined)
                threeMatches++;
            if(region[i].embeddedData.collegeGroup2Id != undefined && region[i].embeddedData.collegeGroup3Id == undefined)
                twoMatches++;
            if(region[i].embeddedData.collegeGroup1Id != undefined && region[i].embeddedData.collegeGroup2Id == undefined)
                oneMatch++;
            else
                zeroMatch++;
        }
        ...
        console.log('********************************************************************************************************');
        console.log('                      Number of colleges matched per candidate in "' + regionName + '" region ');
        console.log('********************************************************************************************************');
